...
const NouveauCmdFrs = () => {
  ...
  const loadFournisseurs = async () => {
    try {
      const fournisseursData = await cltFrsService.findAllFournisseurs();
  ...
  const loadCommande = async (commandeId) => {
    try {
      setIsLoading(true);
      const commande = await cmdCltFrsService.findByIdCommandeFournisseur(commandeId);
      
      if (commande) {
        // Map ligneCommandeFournisseurs to the format expected by the form
        const mappedArticles = (commande.ligneCommandeFournisseurs || []).map(ligne => ({
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    if (!formData.codeCommande || !formData.dateCommande || !formData.fournisseurId) {
    ...
    try {
      ...
      const commandeData = {
        ...
        ligneCommandeFournisseurs: formData.articles.map(article => ({
      ...
      if (isEditing) {
        ...
        await cmdCltFrsService.updateCommandeFournisseur(id, commandeData);
      } else {
        await cmdCltFrsService.saveCommandeFournisseur(commandeData);
      }
